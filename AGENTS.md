...
- If present, check `.memories/context.md` first — project context, constraints, and tech stack.
- If present, check `.memories/knowledge/` — project knowledge base with verified claims at different assurance levels.
- If present, check `.memories/sessions/` — descriptions of past coding sessions.
...
* Configured via **pyproject.toml** (including **commitizen** in this repo).
...
* Target **Python == 3.12.3** (this repo is pinned for Databricks Runtime compatibility).
* Prefer Python 3.12–native features and standard-library solutions where they improve clarity, safety, or performance.
...
* Centralize configuration in **pyproject.toml** for:
  ...
  * commitizen (this repo uses `[tool.commitizen]` in `pyproject.toml`)
...
* **Do not** introduce configuration files outside `pyproject.toml` in this repo. No `.flake8`, no `setup.cfg`, no `tox.ini`, no `mypy.ini`.
...
* **Do not** add dependencies for convenience if the standard library (Python ≥ 3.12) suffices.
